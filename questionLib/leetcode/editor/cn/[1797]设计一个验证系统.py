...
class AuthenticationManager:
    ...
    def generate(self, tokenId: str, currentTime: int) -> None:
        self.data[tokenId] = currentTime

    def renew(self, tokenId: str, currentTime: int) -> None:
        self.clearExpiredTokens(currentTime)
        if tokenId in self.data and currentTime < self.data[tokenId] + self.timeToLive:
            self.data[tokenId] = currentTime
    ...
    def clearExpiredTokens(self, currentTime):
        expired_keys = []
        for k, v in self.data.items():
            if currentTime >= v + self.timeToLive:
                expired_keys.append(k)
        while expired_keys:
            self.data.pop(expired_keys.pop())
        # print('clear:', self.data)
