...
#  
#  Related Topics 数组 模拟 👍 310 👎 0
# 1,2 d=4: 1,2,3,4 2,3,4,5 = 1,2,3,4,5 = 5 4*2-(4-2+1) = 8-3
    ...
    def findPoisonedDuration(self, timeSeries: List[int], duration: int) -> int:
        ...
        for i in range(1, len(timeSeries)):
            if timeSeries[i] - timeSeries[i-1] < duration:
                cutoff += duration-(timeSeries[i]-timeSeries[i-1])

        return duration*len(timeSeries) - cutoff
